...
In the [WPA/WPA2 Authentication](../../networking/wifi/WPA-WPA2.md#Authentication), the first two messages can be used to brute-force the `PSK`. An attacker only needs *Message 1 and Message 2* from the four-way handshake.
## Attack Steps
...
> [!Note]
> **You'll know you've captured a handshake when it says `EAPOL` in the `airodump-ng` traffic next to the client that authenticated to the target AP**
#### 3. Crack the password
Once you've confirmed that you've captured a handshake, you can use `aircrack-ng` along with a wordlist to crack the `PSK`:
```bash
sudo aircrack-ng -w /path/to/dictionary.txt capture-01.cap
```
Successful output will look something like:
![](../CWP-pics/handshake-attack-2.png)
...
###### 3.1.a Convert `.cap` to hashcatable format
